            ...
            with open(self.identifier, 'a') as out:
                ...
                for individual in population.individuals:
                    ...
                    out.write("(num_feats: %d) Indv: %r\nFitness: %s\n\n" %
                                  (len(individual), individual.features, str(individual.fitness)))
...
class NSGA2(MOA):
    """
    Implementation of the multi-objective NSGA II algorithm based on:
    ...
    def _core(self, generation_start: int, generation_end: int):
        ...
        # DEBUGGING
        log.out(message="Population pre sort and crowding", population=self._population)
        ...
        # DEBUGGING
        log.out(message="Population post sort and crowding", population=self._population)
        ...
        for generation in tqdm(range(generation_start, generation_end), desc=info):
            ...
            # DEBUGGING
            log.out(message="Offspring pre-sort and crowding", population=offspring)

            # Evaluate offspring
            offspring = self._evaluate_fitness(population=offspring)

            # DEBUGGING
            log.out(message="Offspring post sort and crowding", population=offspring)

            # Restart parent solutions
            restart_solutions(self._population.fitness)

            # DEBUGGING
            log.out(message="Population after restart rank and crowding", population=self._population)

            # Merge parents and offspring
            parents_offspring = self._population.merge_population(self._population, offspring)

            # DEBUGGING
            log.out(message="After merge offspring and population", population=parents_offspring)
            ...
            # DEBUGGING
            log.out(message="After sort and crowding parents_offspring", population=parents_offspring)

            # Set new population
            self._set_new_population(parents_offspring)

            #  DEBUGGING
            log.out(message="After select new population", population=self._population)
