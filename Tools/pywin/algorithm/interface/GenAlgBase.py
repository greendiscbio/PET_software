...
## Module dependencies
from .._algorithm_init import _GenAlgBase_init
from pywin.population.Individual import Individual
...
class GenAlgBase:
    ...
    def __init__(self, **kwargs):
        """
            __init__(**kwargs)
        """
        kwargs = _GenAlgBase_init(kwargs)
    ...
    def _label_processing(self, label_names: list):
        ...
        when less than 20% of the features belong to the target class (unbalanced problem).
        ...
        if positive_class_freq is None:
            ...
            if positive_class_freq < (counts.sum() / 4):
    ...
    def _create_dataset(self, individual: Individual, X_data: np.ndarray):
        ...
        if np.isnan(X_data.sum()):
            ...
            # Impute imputation values based on a pywin.imputation.ImputationStrategy
