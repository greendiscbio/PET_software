...
## Module dependencies
from .interface.MultiObjective import MOA
from ._algorithm_init import _SPEA2_init
from ..population.Population import Population
from ..utils.moa.hypervolume import hypervolume
from ..utils.moa.fast_non_dominated_sort import fast_non_dominated_sort
...
class Debugging:
    """
    Class that record the evolution of the individuals of the population throughout the execution of
    the algorithm. Implemented only for debugging during development.
    It can be disabled by selecting ENABLED parameter as false.
    """
    ENABLED = False
    ...
    def __init__(self, identifier: str):
        ...
        Notes
        -------
        The algorithm identifier will be the name of the output file.
        """
        if self.ENABLED:
            self.identifier = '%s.log' % identifier
            with open(self.identifier, 'w') as out:
                out.write("LOG FILE: %s\n\n" % self.identifier)

    def out(self, message: str, population: Population):
        """
        Write a log with the message and the population received. The message will only be written if the class
        is enabled.

            out(message: str, population: Population)
        ...
        """
        if self.ENABLED:
            with open(self.identifier, 'a') as out:
                out.write("%s\n\n" % message)

                for individual, fitness in zip(population.individuals, population.fitness):
                    if isinstance(fitness, int):
                        out.write("(num_feats: %d) Indv: %r\nFitness: %s\n\n" %
                                  (len(individual), individual, str(fitness)))
                    else:
                        out.write("(num_feats: %d) Indv: %r\nSolutions: %r\nRank:%s\nCrowding: %s\n\n " %
                                  (len(individual), individual, fitness.values, str(fitness.rank),
                                   str(fitness)))

                out.write("Length population: %d\n==================================="
                          "===================================\n\n" % population.length)
...
class SPEA2(MOA):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        if self.positive_class is not None:
            self._y = self._label_processing(y.copy())
        else:
            self._y = y.copy()
    ...
    def _evaluate_fitness(self, population: Population):
        ...
        for n, individual in enumerate(population.individuals):
            ...
            for fitness_func in self.fitness:
                scores.append(fitness_func.eval_fitness(X=X_data, y=self._y, num_feats=len(population.features)))
    ...
    def _core(self, generation_start: int, generation_end: int):
        ...
        """
        log = Debugging(identifier=self.id)
        ...
        for generation in tqdm(range(generation_start, generation_end), desc=info):
            ...
            log.out(message="After evaluate population", population=self._population)
            ...
            log.out(message="After select pt", population=pt)
            ...
            log.out(message="After fast non dominated sort", population=pt)

            # Assign fitness
            fitness_assignment(pt_fitness)

            log.out(message="After ffitness assignment", population=pt)
            ...
            log.out(message="After density estimation", population=pt)
            ...
            log.out(message="Archive", population=self.archive)

            # Assign archive individuals to population
            self._population.set_new_individuals(archive)

            log.out(message="New population (=Archive)", population=self._population)
            ...
            log.out(message="Population after selection, crossover and mutation", population=self._population)

            log.out(message="Has the archive changed??", population=self.archive)
