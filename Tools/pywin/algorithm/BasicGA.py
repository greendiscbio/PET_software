...
## Module dependencies
from .interface.GenAlgBase import GenAlgBase
from ._algorithm_init import _BasicGA_init
...
class Debugging:
    ...
    the algorithm. Implemented only for debugging.
        ...
        if self.ENABLED:
            with open(self.identifier, 'a') as out:
                ...
                for individual in population.individuals:
                    out.write("\n(num_feats: %d) Individual: %s" % (len(individual), individual))

                out.write("\n\nLength population: %d\n==================================="
                          "===================================\n\n" % population.length)


class BasicGA(GenAlgBase):
    """
    ...
    def __repr__(self):
        return f"BasicGA(population_size={self._population.size} generations={self.generations} " \
    ...
    def best_performance(self):
        """
        Returns the largest value found for the metric selected in pywin.fitness.interface.FitnessStategy
    ...
    def _core(self, generation_start: int, generation_end: int):
        ...
        log.out(message="Initial population", population=self._population)

        # Evaluation of the population
        self._population = self._evaluate_fitness(-1, self._population, annotate=True)

        # DEBUGGING
        log.out(message="After fitness evaluation", population=self._population)
        ...
        for generation in tqdm(range(generation_start, generation_end), desc=info):
            ...
            if self.elitism is not None:
                ...
                # DEBUGGING
                log.out(message="Elite", population=elite)
            ...
            # DEBUGGING
            log.out(message="After annihilation", population=self._population)
            ...
            # DEBUGGING
            log.out(message="After selection", population=best_individuals)

            # Apply crossover
            offspring = self.crossover.cross_population(best_individuals)

            # DEBUGGING
            log.out(message="After crossover", population=offspring)
            ...
            # DEBUGGING
            log.out(message="After mutation", population=offspring)
            ...
            # DEBUGGING
            log.out(message="After merge offspring and elite", population=offspring)

            # Evaluate fitness
            self._population = self._evaluate_fitness(generation, offspring)

            # DEBUGGING
            log.out(message="New population", population=offspring)
