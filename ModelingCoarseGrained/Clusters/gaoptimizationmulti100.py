...
for task in tasks_clusters:
    ...
    if numpy.sum(y_data)!=0:
        par = []
        for k in range(0,10):
            ALG = random.choices(['ALG1','ALG2'],[5/9,4/9],k=1)[0]
            SVM = random.choices(['SVM1','SVM2','SVM3'],[2/9,3/9,4/9],k=1)[0]
            MR = random.choices(['MR1','MR2','MR3'],[4/9,3/9,2/9],k=1)[0]
            SE = random.choices(['SE1','SE2','SE3'],[1/9,5/9,3/9],k=1)[0]
        ...
        try:
            ...
            par_ga.save(dir_name='./Optimization_multi_100/'+str(task),overwrite=True)
            del par, par_ga
        except: pass

#Register the results for each region

f1 = open('./Optimization_multi_100/optimization_train.csv',mode='w')
f2 = open('./Optimization_multi_100/optimization_test.csv',mode='w')
f3 = open('./Optimization_multi_100/optimization_features.csv',mode='w')
...
f2.close()
f3.close()
f1 = open('./Optimization_multi_100/optimization_train.csv',mode='a')
f2 = open('./Optimization_multi_100/optimization_test.csv',mode='a')
f3 = open('./Optimization_multi_100/optimization_features.csv',mode='a')
for task in tasks_clusters:
    try:
        print(task)
        names = [name for name in os.listdir("./Optimization_multi_100/"+str(task))]
        models = [BasicGA.load(model,"./Optimization_multi_100/"+str(task)) for model in names]
        for name,model in zip(names,models):
            for index in range(len(model.best_features)):
                try:
                    ...
                    train.to_csv('./Optimization_multi_100/optimization_train.csv',mode='a',index=False,header=False)
                    test.to_csv('./Optimization_multi_100/optimization_test.csv',mode='a',index=False,header=False)
                    features.to_csv('./Optimization_multi_100/optimization_features.csv',mode='a',index=False,header=False)
                except: pass
    except: pass
f1.close()
f2.close()
...
train = pandas.read_csv('./Optimization_multi_100/optimization_train.csv')
test = pandas.read_csv('./Optimization_multi_100/optimization_test.csv')
features = pandas.read_csv('./Optimization_multi_100/optimization_features.csv')
regions = tasks_clusters
criteria = ['accuracy','f1']
mo.keep_best(train,test,features,regions,criteria,'./Optimization_multi_100/optimization')