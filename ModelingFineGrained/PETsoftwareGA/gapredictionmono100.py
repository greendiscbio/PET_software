...
models = pandas.read_csv('./Optimization_mono_100/optimization_best_features.csv')
...
for replica in range(replicas):
    ...
    for task in tasks_aal:
        try:
            name = models.loc[models['Model'].str.contains(str(task)+'$')]['Model'].values[0]
            model = BasicGA.load(name,"./Optimization_mono_100/"+str(task))
            temp_prediction = pandas.DataFrame({task:Evaluator(model).predictions(cv=folds)})
    ...
    prediction_aal.to_csv('./Prediction_mono_100/Prediction/Replicas/prediction_aal_rep_'+str(replica)+'.csv',mode='w',index=False)
    ...
    prediction_brodmann.to_csv('./Prediction_mono_100/Prediction/Replicas/prediction_brodmann_rep_'+str(replica)+'.csv',mode='w',index=False)
...
consensus_prediction_aal.to_csv('./Prediction_mono_100/Prediction/prediction_aal.csv',mode='w',index=False)
consensus_prediction_brodmann.to_csv('./Prediction_mono_100/Prediction/prediction_brodmann.csv',mode='w',index=False)
...
for relevance_threshold in relevance_thresholds:
    ...
    for replica in range(replicas):
        print(replica)
        prediction_aal = pandas.read_csv('./Prediction_mono_100/Prediction/Replicas/prediction_aal_rep_'+str(replica)+'.csv')
        ...
        correction_aal.to_csv('./Prediction_mono_100/Absolute/Replicas/correction_aal_'+str(relevance_threshold)+'_rep_'+str(replica)+'.csv',mode='w',index=False)
        try: consensus_correction_aal = consensus_correction_aal + correction_aal
        except: consensus_correction_aal = correction_aal
        prediction_brodmann = pandas.read_csv('./Prediction_mono_100/Prediction/Replicas/prediction_brodmann_rep_'+str(replica)+'.csv')
        ...
        correction_brodmann.to_csv('./Prediction_mono_100/Absolute/Replicas/correction_brodmann_'+str(relevance_threshold)+'_rep_'+str(replica)+'.csv',mode='w',index=False)
    ...
    consensus_correction_aal.to_csv('./Prediction_mono_100/Absolute/correction_aal_'+str(relevance_threshold)+'.csv',mode='w',index=False)
    consensus_correction_brodmann.to_csv('./Prediction_mono_100/Absolute/correction_brodmann_'+str(relevance_threshold)+'.csv',mode='w',index=False)
...
for relevance_threshold in relevance_thresholds:
    ...
    for replica in range(replicas):
        ...
        correction_aal_CT = ra.apply_rules(prediction_aal_CT,rules_aal_CT,'hypometabolic','hypometabolic')
        correction_aal = pandas.concat([correction_aal_AD,correction_aal_FTD,correction_aal_CT],axis=0).sort_index()
        correction_aal.to_csv('./Prediction_mono_100/Normalised/Replicas/correction_aal_'+str(relevance_threshold)+'_rep_'+str(replica)+'.csv',mode='w',index=False)
        try: consensus_correction_aal = consensus_correction_aal + correction_aal
        except: consensus_correction_aal = correction_aal
        prediction_brodmann = pandas.read_csv('./Prediction_mono_100/Prediction/Replicas/prediction_brodmann_rep_'+str(replica)+'.csv')
        prediction_brodmann_AD = prediction_brodmann.iloc[Y_brodmann_quan_AD.index,]
        ...
        correction_brodmann_CT = ra.apply_rules(prediction_brodmann_CT,rules_brodmann_CT,'hypometabolic','hypometabolic')
        correction_brodmann = pandas.concat([correction_brodmann_AD,correction_brodmann_FTD,correction_brodmann_CT],axis=0).sort_index()
        correction_brodmann.to_csv('./Prediction_mono_100/Normalised/Replicas/correction_brodmann_'+str(relevance_threshold)+'_rep_'+str(replica)+'.csv',mode='w',index=False)
        try: consensus_correction_brodmann = consensus_correction_brodmann + correction_brodmann
        except: consensus_correction_brodmann = correction_brodmann
    consensus_correction_aal[consensus_correction_aal<=replicas/2]=0
    consensus_correction_aal[consensus_correction_aal>replicas/2]=1
    consensus_correction_brodmann[consensus_correction_brodmann<=replicas/2]=0
    consensus_correction_brodmann[consensus_correction_brodmann>replicas/2]=1
    consensus_correction_aal.to_csv('./Prediction_mono_100/Normalised/correction_aal_'+str(relevance_threshold)+'.csv',mode='w',index=False)
    consensus_correction_brodmann.to_csv('./Prediction_mono_100/Normalised/correction_brodmann_'+str(relevance_threshold)+'.csv',mode='w',index=False)
